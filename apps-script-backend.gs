 ...
 *
 * 賽制：
 *   一圈 = 成美長壽橋籃球場 → 成功橋（過橋走對岸）→ 籃球場 ＝ 3K，跑滿 4 圈 ＝ 12K
 *   個人組   1 人跑 4 圈   · 22 個名額
...
const EVENT_ROUTE    = '成美長壽橋籃球場 → 沿基隆河跑到成功橋 → 過橋沿對岸河濱跑回籃球場 ＝ 一圈 3K，跑滿 4 圈 ＝ 12K';
// 合作廠商
...
function printPage_(rows) {
  ...
  html += '<h1>' + esc_(EVENT_NAME) + '｜工作人員列印頁</h1>'
    ...
    + '· 沿基隆河繞一圈（過成功橋走對岸），每跑回成美長壽橋籃球場算一圈，依編碼在計圈表畫正字；個人組 4 圈、兩人接力每人 2 圈、四人接力每人 1 圈。<br>'
