...
function teamTable_(d, teamNo) {
  // 參賽者的提醒信不列隊友的緊急聯絡人（個資）；主辦通知信仍然要列
  const showEmg = !d.hideEmg;
  let html = '<table style="border-collapse:collapse;font-size:14px">'
    ...
    + (showEmg ? '<td style="border:1px solid #ddd;background:#f7f7f7;padding:6px"><b>緊急聯絡人</b></td>' : '')
  ...
  d.members.forEach(function (m) {
    html += '<tr>'
      ...
      + (showEmg ? '<td style="border:1px solid #ddd;padding:6px">' + esc_(m.emgName) + ' ' + esc_(m.emgPhone) + '</td>' : '')
...
// 從三個分頁的資料反推出寄信要用的隊伍物件。
// 報名當下是由 validate_() 產生 d，重寄時只能從表上回推。
function teamsFromRows_(rows) {
  const map = {};
  const order = [];
  rows.forEach(function (r) {
    const cat    = String(r[C['組別'] - 1] || '').trim();
    const teamNo = String(r[C['隊伍編號'] - 1] || '').trim();
    if (!CATEGORIES[cat] || !teamNo) return;
    const key = cat + '||' + teamNo;
    if (!map[key]) {
      map[key] = {
        category: cat,
        teamNo:   teamNo,
        teamName: String(r[C['隊伍名稱'] - 1] || '').trim(),
        members:  []
      };
      order.push(key);
    }
    map[key].members.push({
      ...
      email:    String(r[C['Email'] - 1] || '').trim(),
      phone:    String(r[C['電話'] - 1] || '').trim(),
      emgName:  String(r[C['緊急聯絡人'] - 1] || '').trim(),
      emgPhone: String(r[C['緊急聯絡電話'] - 1] || '').trim(),
      fee:      Number(r[C['金額'] - 1]) || 0
    });
  });
  return order.map(function (k) {
    const t = map[k];
    t.members.sort(function (a, b) { return a.seat - b.seat; });   // 隊友表照棒次列
    t.feeTotal = t.members.reduce(function (s, m) { return s + m.fee; }, 0);
    return t;
  });
}

// 沿用 runnerBody_，只在最前面加一段說明；不然收到「報名成功」會以為自己又報了一次
function resendBody_(d, m) {
  return '<div style="font-size:15px;line-height:1.7;background:#fff8e1;'
       + 'border:2px solid #d63a6a;border-radius:10px;padding:12px 14px;margin-bottom:14px">'
       + '<b>這是活動前的提醒信，同時也是最終名單確認，你不需要再報名一次。</b><br>'
       + '請核對以下資料，若有錯誤請在活動前私訊 '
       + '<a href="' + esc_(LINE_OFFICIAL_URL) + '">NID 官方 LINE</a>。'
       + '</div>'
       + runnerBody_(dHideEmg_(d), d.teamNo, m);
}

// 提醒信用的淺複製：只加上「不要列緊急聯絡人」的旗標，不動到原物件
function dHideEmg_(d) {
  const c = {};
...
function resendSubject_(d, m) {
  return '活動提醒｜' + EVENT_NAME + '　編碼 ' + bibCode_(d.category, d.teamNo, m.seat);
}

// 版面測試：三個組別各抽一隊，一律只寄到主辦信箱，不會寄給任何參賽者
function testResend() {
  const teams = teamsFromRows_(allRows_());
  const log = [];
  CATEGORY_ORDER.forEach(function (cat) {
    const d = teams.filter(function (x) { return x.category === cat; })[0];
    if (!d) return;
    const m = d.members[0];
    MailApp.sendEmail({
      to: ADMIN_EMAIL,                     // 測試信一律寄主辦，不碰參賽者信箱
      subject: '[版面測試] ' + resendSubject_(d, m),
      htmlBody: resendBody_(d, m),
      name: SENDER_NAME
    });
    log.push(cat + ' ' + d.teamNo + '（' + m.name + '・' + d.members.length + ' 人）');
  });
  const msg = '已寄 ' + log.length + ' 封版面測試信到 ' + ADMIN_EMAIL + '：' + log.join('、');
  Logger.log(msg);
  return msg;
}

// 寄提醒信：每位參賽者一封，寄到本人信箱。
// only = 隊伍編號陣列（例如 ['B10']）只寄那幾隊；傳 null 就是全部。
// 刻意不用 Email 去重 —— 表上有共用聯絡資料的情況，去重會漏人；每列一封才不會少寄。
function sendReminders_(only) {
  const teams = teamsFromRows_(allRows_());
  let sent = 0, skipped = 0;
  teams.forEach(function (d) {
    if (only && only.indexOf(d.teamNo) === -1) return;
    d.members.forEach(function (m) {
      if (!isEmail_(m.email)) {
        skipped++;
        Logger.log('略過（Email 無效）' + d.category + ' ' + d.teamNo + ' 第 ' + m.seat + ' 棒：' + m.name);
        return;
      }
      try {
        MailApp.sendEmail({
          to: m.email,
          subject: resendSubject_(d, m),
          htmlBody: resendBody_(d, m),
          name: SENDER_NAME
        });
        sent++;
        ...
        skipped++;
        Logger.log('重寄失敗 ' + d.category + ' ' + d.teamNo + ' 第 ' + m.seat + ' 棒：' + err);
      }
    });
  });
  const msg = '已寄 ' + sent + ' 封'
    + (only ? '（只寄 ' + only.join('、') + '）' : '（全部）')
    + '，略過 ' + skipped + ' 封（詳見執行記錄）';
  Logger.log(msg);
  return msg;
}
...
// 全部重寄
function resendAll() {
  return sendReminders_(null);
}

// 只補寄指定隊伍 —— 要補寄誰就改這個清單，然後執行這支
function resendTeams() {
  return sendReminders_(['B10']);
}
